...
****/
{
	...
	while (hd!=(header*)cc->newram && i<epos) {		// replace by cc->extra_start
		...
		for (k=0,hd1=args; k<nargu; k++,hd1=nextof(hd1)) {
			udf_arg* arg=(udf_arg*)p;
	...
	while (CC_ISSET(cc,CC_EXEC_UDF)) {
		...
		if (CC_ISSET(cc,CC_EXEC_RETURN)) {
			...
			/* eliminate all references from result, necessary
			   when the function returns only one result.
			 */
...
static void make_complex (Calc *cc, header *hd)
/**** make_complex
...
{	header *st=hd,*result;
	...
	if (hd->type==s_real || hd->type==s_matrix) {
		if (hd1->type==s_real || hd1->type==s_matrix) {
			...
			if (r!=0 && (c!=c1 || r+r1<0)) cc_error(cc,"columns must agree");
			result=new_matrix(cc,r+r1,c1,"");
			if (r!=0)
...
{	header *st=hd,*result;
	...
	if (hd->type==s_string && hd1->type==s_string) {
		...
		if (hd1->type==s_real || hd1->type==s_matrix) {
			getmatrix(hd,&r,&c,&m);
			getmatrix(hd1,&r1,&c1,&m1);
			if (c!=0 && (r!=r1 || c+c1<0)) cc_error(cc,"rows must agree");
...
{	real vinit,vstep,vend,*m;
	...
	count=(int)(floor(fabs(vend-vinit)/fabs(vstep)+1+cc->epsilon));
...
{	header *hd1,*result=NULL;
	...
	if (hd->type==s_matrix) {
		...
		if (*m<0 || *m>INT_MAX || *(m+1)<0 || *(m+1)>INT_MAX)
			cc_error(cc,"1st arg: matrix size required");
		r1=(int)*m; c1=(int)*(m+1);
		if (hd1->type==s_real) {
		...
		} else cc_error(cc,"2nd arg: real or complex value required");
	} else cc_error(cc,"1st arg: 1x2 real vector required");
	return pushresults(cc,result);
}

header* mzerosmat (Calc *cc, header *hd)
{	header *result;
	real rows,cols,*m;
	int r,c;
	ULONG i,n;
...
{	header *st=hd,*result=NULL,*hd1,*hd2=0;
	...
	hd=getvalue(cc,hd);
	if (hd->type!=s_matrix || dimsof(hd)->r!=1 || dimsof(hd)->c!=2)
		cc_error(cc,"1st arg: 1x2 real matrix expected");
	rows=*matrixof(hd); cols=*(matrixof(hd)+1);
	if (rows<0 || rows>=INT_MAX || cols<0 || cols>=INT_MAX)
		cc_error(cc,"bad size required");
	r=(int)rows; c=(int)cols;
	...
	if (hd2->type==s_matrix || hd2->type==s_real) {
		...
		if (rd!=1 || cd<1) cc_error(cc,"3rd arg: row vector expected");
		m=matrixof(result);
		for (i=0; i<r; i++) {
			if (i+k>=0 && i+k<c) {
				*mat(m,c,i,i+k)=*md;
...
{	header *st=hd,*hd1,*result;
	...
	} else {	result=new_matrix(cc,1,c1,"");
		mr=matrixof(result);
...
restart:
	...
	spread1(cc,rddeval,cddeval,hd1);
	return pushresults(cc,hd1);
}
...
static void fft (Calc *cc, double *a, int n, int signum)
{	cplx z;
	double h;
	int i;
